...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Requires: run BTG Backend/supabase-public-data.sql once (grants anon
   SELECT + RLS read policies on the public tables).

   Runs daily via .github/workflows/update-db-cache.yml (and on demand via
   workflow_dispatch). Also useful locally:
     node BTG Backend/scripts/build-db-cache.js

   Output:
     Site DB/cache/public-data.json
       { generatedAt, version,
         series, teams, race_seasons, races, race_results, race_sprints,
         race_qualifying, race_pit_stops, race_grid_penalties,
         season_driver_standings, season_team_standings,
         race_car_performance, team_engine_history, team_staff, team_part_stats }
   ═══════════════════════════════════════════════════════════════════════════ */
...
const SITE_ROOT = path.resolve(process.env.BTG_SITE_ROOT || (function () {
  // Local: BTG Backend/scripts → ../.. /Site DB. GitHub repo: scripts/ is at the
  // repo root and the cache lives at the repo root (no Site DB folder), so fall
  // back to ../ (the repo root) when the local layout isn't present.
  ...
  if (directFailed) {
    ...
    try {
      ...
      if (snap && Array.isArray(snap.races)) {
        Object.keys(snap).forEach(function (k) { if (Array.isArray(snap[k])) data[k] = snap[k]; });
