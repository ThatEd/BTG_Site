...
BTG.teamLogoCandidates = function(teamName, series) {
  ...
  // 5. Explicit mini map (already points at the right mini file)
  var mini = BTG.TEAM_MINI_LOGO[teamName];
...
BTG.driverPhoto = function(fullName, base) {
  ...
  // Normalize a name token → lowercase, non-alphanumerics → single underscore
  function tok(s) {
...
/**
 * Build an <img> tag for a driver photo, with onerror fallback across
 * .webp → .png → the "?" placeholder span.
...
BTG.driverPhotoImg = function(fullName, size, className) {
  var base = BTG.driverPhoto(fullName);
  var sizePx = size || 76;
  ...
  var webp = base + '.webp';
  var png = base + '.png';
  // onerror: try png, then swap in the "?" placeholder
  return '<img class="' + cls + '" src="' + webp + '" width="' + sizePx + '" height="' + sizePx + '" alt="' + BTG.esc(fullName || '') + '" '
    + 'onerror="if(!this._t){this._t=1;this.src=this.dataset.png;}else{BTG.driverPhotoFail(this);}" '
    + 'onload="this.style.opacity=1;" style="opacity:0;" '
    + 'data-png="' + png + '" data-name="' + BTG.esc(fullName || '') + '">';
};

/**
 * Global fallback: replace a driver photo <img> with the "?" placeholder
 * when both .webp and .png were tried and failed.
 */
